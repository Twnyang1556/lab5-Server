/**
 * This file provides an implementation of a Socket and SocketAdaptor using
 * basic TCP. You will want to understand what is going on in this file for
 * Task 2.1, but no modifications to this file should be necessary.
 */
...
TCPSocket::TCPSocket(int port_no, struct sockaddr_in addr)
    : _socket(port_no), _addr(addr) {
...
void TCPSocket::write(char const *const buf, const size_t buf_len) {
  ...
  if (ret_code == -1) {
    ...
    throw ConnectionError("Could not write all bytes of: \'" +
                          buf_hex_stream.str() + "\'. Expected " +
                          std::to_string(buf_len) + " but actually sent " +
